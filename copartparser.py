...
async def fetch_body_class_from_nhtsa(vin: str, client: httpx.AsyncClient) -> Optional[str]:
    """
    Достаёт Body Class из NHTSA DecodeVinExtended по VIN.
    Возвращает нормализованный Body Class или None.
    """
    ...
    try:
        ...
        print(f"⚠️ NHTSA error for VIN {vin}: {e}")
        return None

    try:
        data = resp.json()
    except Exception as e:
        print(f"⚠️ NHTSA JSON parse error for VIN {vin}: {e}")
        return None
...
class CopartBot:
    ...
    async def _gather_all_copart_thumbnails(self, max_clicks: int = 30) -> list[str]:
        """
        Собирает ВСЕ картинки Copart для лота, листая слайдер стрелкой
        'thumbnail-next-image-icon' и проходясь по всем <img>.

        Возвращает список thumbnail-URL'ов вида *_thb.jpg (уникальных).
        """

        async def grab_all_copart_imgs() -> list[str]:
            """
            Берём все <img>, у которых src указывает на copart ids-c-prod-lpp.
            """
            try:
        ...
        # Собрали стартовое состояние
        seen_raw: set[str] = set(await grab_all_copart_imgs())
        print(f"  🖼️ Стартовых Copart-URL (все img): {len(seen_raw)}")

        # Листаем основной слайдер
        no_new_streak = 0
        ...
        for i in range(max_clicks):
            ...
            if not visible:
                print(f"  🔚 Кнопка next-image не видна, стоп (клик {i+1}/{max_clicks})")
                break

            print(f"  🔁 Кликаю next-image (клик {i+1}/{max_clicks})...")
            try:
                await btn.click()
            except Exception as e:
                print(f"  ⚠️ Не удалось кликнуть по next-image: {e}")
                break
            ...
            print(f"  ➕ Новых Copart-URL после клика: {len(new_urls)}")

            if not new_urls:
                no_new_streak += 1
                if no_new_streak >= 3:
                    print("  🔚 Уже несколько кликов нет новых картинок, выхожу.")
                    break
            else:
                no_new_streak = 0
                seen_raw |= new_urls

        print(f"  ✅ Всего уникальных Copart-URL (сырых): {len(seen_raw)}")

        # Нормализуем в формат *_thb.*
        thumbs: set[str] = set()
        ...
        for url in seen_raw:
            ...
            # Если это уже thumbnail
            if "_thb" in u:
                thumbs.add(u)
                continue

            # Если это *_ful или *_hrs — приводим к *_thb
            ...
            # Если вообще без суффикса — добавим _thb как fallback
            base, ext = os.path.splitext(u)
            thumbs.add(f"{base}_thb{ext or '.jpg'}")

        thumbs_list = list(dict.fromkeys(sorted(thumbs)))
        print(f"  🎯 Итого thumbnail-URL (*_thb): {len(thumbs_list)}")
        return thumbs_list
    ...
    async def get_lot_details(self, lot_url: str) -> Dict[str, Any]:
        """
        Открывает страницу лота и вытягивает ключевые поля + ссылки на картинки.
        """
        ...
        # Дождёмся появления хотя бы одной картинки (чтобы галерея инициализировалась)
        try:
            await self.page.wait_for_selector(
        ...
        # 2) Собираем все copart-картинки через листание слайдера
...
def build_image_sets(images: List[str]) -> tuple[List[str], List[str], Optional[str]]:
    ...
    small  — оригинальные thumbnail'ы (_thb).
    hd     — только high-res варианты (_hrs) для каждого thumbnail.
    """
    thumbs: List[str] = []
    hd: List[str] = []
    ...
    for url in images or []:
        ...
        # сохраняем исходный thumbnail
        thumbs.append(url)

        # если это thumbnail вида ..._thb.jpg — строим ссылку на HD
        if "_thb" in url:
            ...
            # 🔥 оставляем только HRS (самое лучшее качество)
        ...
        for idx, url in enumerate(links, start=1):
            ...
            try:
                ...
                if len(vin_raw) == 17:
                    ...
                    if body_class:
                        # сохраняем отдельным ключом, чтобы было видно источник
                        details["body_type_nhtsa"] = body_class
                        print(f"  🧬 NHTSA Body Class для VIN {vin_raw}: {body_class}")
                # -------------------------------------

                # ---------- DEBUG: что пришло с Copart ----------
                copart_imgs: List[str] = details.get("images") or []
                print(f"  🖼️ Copart thumbnails ({len(copart_imgs)} шт.):")
                ...
                # ---------- DEBUG: что получилось в S3 ----------
                print(f"  ☁️ S3 small ({len(s3_small)} шт.):")
                for img_url in s3_small:
                    print(f"      {img_url}")

                print(f"  ☁️ S3 HD ({len(s3_hd)} шт.):")
...
async def main():
    print(f"⚙️ START_LINK_INDEX = {START_LINK_INDEX}")
    ...
    try:
        ...
        print(f"\n🔢 Всего ссылок на лоты: {original_total_links}")

        if original_total_links:
            print(f"   Первый URL до обрезки: {links[0]}")

        # 🔢 Если нужно начать не с первой, а, например, с 540-й
        global_start_index = 0  # глобальный сдвиг по оригинальному списку

        if START_LINK_INDEX > 0:
            if START_LINK_INDEX >= original_total_links:
                print(
                    f"⚠️ START_LINK_INDEX={START_LINK_INDEX} больше количества ссылок "
                    f"({original_total_links}), обрабатывать нечего."
                )
                return
            ...
            print(
                f"⏩ Пропускаю первые {START_LINK_INDEX} ссылок, "
                f"начинаю с {START_LINK_INDEX + 1}-й (1-based нумерация в исходном списке)."
            )
            # Обрезаем список ссылок — дальше работаем только с «хвостом»
            links = links[START_LINK_INDEX:]
            print(f"   Первый URL ПОСЛЕ обрезки: {links[0]}")

        total_links = len(links)
        print(f"🔢 Ссылок к обработке после обрезки: {total_links}")
        ...
        for start in range(0, total_links, BATCH_SIZE):
            ...
            # глобальные индексы относительно исходного списка (0-based → 1-based)
            global_index_start = global_start_index + start + 1
            global_index_end = global_start_index + start + len(batch_links)

            print(
                f"\n🚀 Обработка батча ссылок {global_index_start}–{global_index_end} "
                f"из {original_total_links}"
            )

            # 3.1) ТЯНЕМ ДЕТАЛИ ПО БАТЧУ ССЫЛОК
            # (внутри уже дергается NHTSA + картинки → S3)
